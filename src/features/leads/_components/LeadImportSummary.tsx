...
type SummaryCardProps = {
  label: string;
  count: number;
  color: "red" | "amber" | "orange" | "sky" | "emerald" | "slate";
...
const colorMap = {
  ...
  orange: {
    ...
    activeBg: "bg-orange-100 dark:bg-orange-950/40 ring-2 ring-orange-400",
    count: "text-orange-950 dark:text-orange-100",
    label: "text-orange-800/70 dark:text-orange-300/70",
  },
  sky: {
...
function SummaryCard({ label, count, color, icon, active, onClick }: SummaryCardProps) {
...
type LeadImportSummaryProps = {
  result: LeadImportResult;
  activeTab: string;
  onTabChange: (tab: string) => void;
};

export function LeadImportSummary({ result, activeTab, onTabChange }: LeadImportSummaryProps) {
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Import Results</h2>
      ...
      <div className="overflow-x-auto pb-2">
        <div className="flex gap-3" style={{ minWidth: "1050px" }}>
          ...
          <SummaryCard label="Company Not Exist" count={result.companyNotExistCount} color="orange" icon={<Building2 size={14} className="text-orange-500" />} active={activeTab === "companyNotExist"} onClick={() => onTabChange("companyNotExist")} />
          <SummaryCard label="Duplicate" count={result.duplicateCount} color="sky" icon={<AlertTriangle size={14} className="text-sky-500" />} active={activeTab === "duplicate"} onClick={() => onTabChange("duplicate")} />
          <SummaryCard label="Imported" count={result.importedCount} color="emerald" icon={<CheckCircle size={14} className="text-emerald-500" />} active={activeTab === "valid"} onClick={() => onTabChange("valid")} />
        </div>
      </div>
    </div>
  );
